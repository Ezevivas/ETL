...
if bl_Carga_GT:
    ...
    print('se Cargaron las bases CABA, CBA y GT')
else:
    # Union Multiboldt & CBA
    df_AADD = df_AADD_CABA.unionByName(df_AADD_CBA)
    print('se Cargaron las bases CABA y CBA')
...
# display(df_AADD.groupBy(
#             col('partner_id'), 
#             date_format(col('created_date'), 'yyyy-MM').alias('mes'), 
#             col('balance_type_id'), 
#             col('operation_type_id')
#         )
#         .agg(sum(col('row_amount')).alias('monto_orig'))
#         .orderBy('partner_id', 'mes', 'balance_type_id', 'operation_type_id')
# )

### Query ###
# SELECT partner_id, date_format(created_date, 'yyyy-MM') AS mes, balance_type_id, operation_type_id, SUM(row_amount) AS monto_bplay
# FROM default.apuestas_deportivas
# GROUP BY partner_id, date_format(created_date, 'yyyy-MM'), balance_type_id, operation_type_id
# ORDER BY partner_id, mes, balance_type_id, operation_type_id
