...
# MAGIC %md
# MAGIC # Notebook_Upsert_Modelo_PySpark.ipynb
# MAGIC <h> by PySpark
# MAGIC <p> Descripcion: El notebook esta pensado para leer los archivos parquet ya con previo accesos y permisos, de los diferentes origenes, adaptarlos a la necesidad del negocio y cargarlo de manera incremental para el ahorro de recursos.
# MAGIC <p> Se tomará la información desde el STG1 (1ra Capa) de Datalake para generar un consumible de STG2 (2da Capa) en formato Delta Table.
# MAGIC <p> Inputs:
# MAGIC   <ol>
# MAGIC     <li>Tabla 1</li>
# MAGIC     <li>Tabla 2</li>
...
# MAGIC     <li>Tabla 4</li>
# MAGIC   </ol>
# MAGIC <p> Output: Tabla procesada en formato Delta Table actualizada y su carga en el Warehouse de Databricks.
...
# MAGIC   <li>Se definen los paths de los archivos parquet necesarios para cada Base de Datos.</li>
...
# Esta variable se puede definir como variable de entorno, por lo que se quitaria esta celda

if bl_Carga_Historica:
    print("se realizara la carga historica")
else:
...
# COMMAND ----------
  ...
  # Get DW2
df_cr2 = spark.read.parquet('/path/DW2/cr')
df_c2 = spark.read.parquet('/path/DW2/c')
df_csn2 = spark.read.parquet('/path/DW2/csn')
df_co2 = spark.read.parquet('/path/DW2/co')
df_cosn2 = spark.read.parquet('/path/DW2/cosn')
df_e2 = spark.read.parquet('/path/DW2/e')
df_bt2 = spark.read.parquet('/path/DW2/bt')
df_pfb2 = spark.read.parquet('/path/DW2/pfb')
df_fbc2 = spark.read.parquet('/path/DW2/fbc')
df_p2 = spark.read.parquet('/path/DW2/p')
df_btn2 = spark.read.parquet('/path/DW2/btn')

  # Get DW3
df_cr3 = spark.read.parquet('/path/DW3/cr')
df_c3 = spark.read.parquet('/path/DW3/c')
df_csn3 = spark.read.parquet('/path/DW3/csn')
df_co3 = spark.read.parquet('/path/DW3/co')
df_cosn3 = spark.read.parquet('/path/DW3/cosn')
df_e3 = spark.read.parquet('/path/DW3/e')
df_bt3 = spark.read.parquet('/path/DW3/bt')
df_pfb3 = spark.read.parquet('/path/DW3/pfb')
df_fbc3 = spark.read.parquet('/path/DW3/fbc')
df_p3 = spark.read.parquet('/path/DW3/p')
df_btn3 = spark.read.parquet('/path/DW3/btn')

if bl_Carga_Historica:
    print("se realizara la carga historica de Movement y Operation")
      # Get DF Movement
    df_m2 = spark.read.parquet('/path/DW2/movement/')
    df_m3 = spark.read.parquet('/path/DW3/movement/')
      # Get DF Operation
    df_o2 = spark.read.parquet('/path/DW2/operation/')
    df_o3 = spark.read.parquet('/path/DW3/operation/')
else:
    print("Se actualizara el año corriente")
      # Get DF Movement
    df_m2 = spark.read.parquet('/path/DW2/movement/Year=2025/*')
    df_m3 = spark.read.parquet('/path/DW3/movement/Year=2025/*')

      # Get DF Operation
    df_o2 = spark.read.parquet('/path/DW2/operation/Year=2025/*')
    df_o3 = spark.read.parquet('/path/DW3/operation/Year=2025/*')

# COMMAND ----------

try:
      #Get Nuevo DW9
    df_bet = spark.read.parquet('/path/DW9/bets')
      #Get Tickets GT
    df_tick = spark.read.parquet('/path/DW9/tickets')
      #Get Users GT
    df_usr = spark.read.parquet('/path/DW9/users')
      # Get Bet Selections
    df_bs = spark.read.parquet('/path/DW9/bet_selections')
      # Get Channel login activities
    df_la = spark.read.parquet('/path/DW9/login_activities')
      # Get Result Statuses
    df_rs = spark.read.parquet('/path/DW9/result_statuses')
      # Get Translation Entries
    df_te = spark.read.parquet('/path/DW9/translation_entries')
      # Get Markets
    df_m = spark.read.parquet('/path/DW9/markets')
      # Get Selections
    df_s = spark.read.parquet('/path/DW9/selections')
      # Get Fixtures
    df_f = spark.read.parquet('/path/DW9/fixtures')
    
    ### Switch Carga Nuevo DW9
    bl_Carga_DW9 = True

# Control de excepciones/errores en la carga
...
# Definicion de parametros para una asignacion en Casos de Migraciones
# Definir los datos
data = [(11, 110), (12, 120), (99, 256)]
...
# Crear el DataFrame
df_partner_Migracion = spark.createDataFrame(data, columns)
...
#   Path PRD del Delta Table
path_delta_table_prd = "path.produccion.NombreTabla"

#   Path DEV del Delta Table
path_delta_table_dev = "path.desarrollo.NombreTabla"
...
#   Switch PROD/DEV
if PROD:
    path_delta_table = path_delta_table_prd
else:
    path_delta_table = path_delta_table_dev

# Manejar el caso en que la tabla no exista
try:
    # Intentar leer la tabla Delta
    df_origen = spark.read.table(path_delta_table)
...
def Create_Table_DW(cr, c, co, p, o, m, e, bt, pfb, fbc):
...
def Create_AADD_Efectivo_NewDW(bet, tick, usr, bs, la, rs, te, s, m, f, p):
    ...
    Ap_Ef_AADD = bet.filter(~col("result_status_id").isin(8, 9, 10, 11)).filter(col("amount").isNotNull()).alias("b_Ap")\
        ...
        concat(p["partner"], when(usr_f['alira_id'].isNotNull(), usr_f['alira_id'])
            .when(usr_f['bookmaker_id'].isin(88), concat(usr_f['bookmaker_id'], format_string("%08d", usr_f["id"])))
                .otherwise(usr_f["id"])).cast(LongType()).alias("partner_player_id"), ##### Contempla Migracion ####        
...
def Create_AADD_Bono_NewDW(bet, tick, usr, bs, la, rs, te, s, m, f, p):
    ...
    Ap_Bo_AADD = bet.filter(~col("result_status_id").isin(8, 9, 10, 11)).filter(col("bonus_user_amount").isNotNull()).alias("bet")\
        ...
        concat(p["partner"], when(usr_filter['alira_id'].isNotNull(), usr_filter['alira_id'])
                                .when(usr_filter['bookmaker_id'].isin(88), concat(usr_filter['bookmaker_id'], format_string("%08d", usr_filter["id"])))
                                    .otherwise(usr_filter["id"])).cast(LongType()).alias("partner_player_id"),  ##### Contempla Migracion ####
        concat(p["partner"], tick['id'], bet['id']).cast(LongType()).alias('partner_coupon_row_id'),
        p["partner"].cast(LongType()).alias('partner_id'),
        tick['id'].cast(LongType()).alias('coupon_id'),
        bet['id'].alias('row_id'),
...
# Apuestas Deportivas DW2
df_AADD_DW2 = Create_Table_DW(df_cr2, df_c2, df_co2, df_p2, df_o2, df_m2, df_e2, df_bt2, df_pfb2, df_fbc2)
# Apuestas Deportivas DW3
df_AADD_DW3  = Create_Table_DW(df_cr3, df_c3, df_co3, df_p3, df_o3, df_m3, df_e3, df_bt3, df_pfb3, df_fbc3)

if bl_Carga_GT:
    # Apuestas Deportivas GT PRN & JUJ
    df_AD_Bono_NDW = Create_AADD_Bono_NewDW(df_bet, df_tick, df_usr, df_bs, df_la, df_rs, df_te, df_s, df_m, df_f, df_partner_Migra)
    df_AD_Eftvo_NDW = Create_AADD_Efectivo_NewDW(df_bet, df_tick, df_usr, df_bs, df_la, df_rs, df_te, df_s, df_m, df_f, df_partner_Migra)

    df_AADD_NDW = df_AD_Bono_NDW.unionByName(df_AD_Eftvo_NDW)
...
if bl_Carga_GT:
    ...
    df_AADD = df_AADD_DW2.unionByName(df_AADD_DW3).unionByName(df_AADD_NDW)
    print('se Cargaron las bases DW2, DW3 y Nuevo DW9')
else:
    # Union Multiboldt & CBA
    df_AADD = df_AADD_DW2.unionByName(df_AADD_DW3)
    print('se Cargaron las bases DW2 y DW3')
...
def filter_last_day(df): 
    ...
    result = df.filter((col("created_date") <= yesterday_end)) #.withColumn("fecha_actualizacion", current_timestamp())
    return result
...
# Esta configuracion se utiliza en los casos donde se debe escalar el cluster de procesamiento por cuestiones de volumenes de datos.
...
# Filtra los ultimos 20 días para la actualizacion incremental(Upsert)
...
###########################################################
#### Este codigo previene los errores de actualizacion ####
###########################################################
## Elimina los ultimos 10 días de carga
def delete_recent_rows(table_name: str, days: int = 10):
    # Referencia a la tabla Delta
    delta_table = DeltaTable.forName(spark, table_name)
    
    # Construcción de la condición de borrado
    delete_condition = f"fecha >= current_date() - interval {days} days"
    
    # Ejecutar el delete
    delta_table.delete(delete_condition)
    print(f"Se eliminaron registros de los últimos {days} días de la tabla '{table_name}'.")

# Ejemplo de uso
delete_recent_rows(delta_table_FCR, days=10)
...
# Obtiene la tabla existente de AADD y filtra la columna de "fecha_actualizacion"
df_existente = spark.read.table(delta_table_FCR).select('created_date',
'resolution_date', 'fecha', 'partner_player_id', 'partner_coupon_row_id', 'partner_id', 'coupon_id', 'row_id', 'bet_size', 'cant_ap_evento', 'evento_unico', 'max_resolution_date_comb', 'estimated_end_date', 'max_event_end_date', 'bet_type_id', 'live_betting_id', 'coupon_channel', 'partner_event_id', 'event_id', 'resolucion', 'market', 'bet_offer_description', 'row_odds', 'combination_odds', 'combination_stake', 'row_amount', 'balance_type_id', 'combination_status_id', 'operation_type_id', 'room_id', 'provider_id', 'cupon_externo', 'freebet_configuration_id', 'freebet_configuration', 'diccionario_aadd')

# Filtra el df calculado en el notebook a partir del ultimo dia de ejecucion
df_calculado = filter_last_day(df_AADD)

# Calcula la diferencia entre el df existente y el df calculado y le agrega la columna de "fecha_actualizacion"
df_diferencia_except = (df_calculado.exceptAll(df_existente)).withColumn("fecha_actualizacion", current_timestamp()).persist()

# Filtra los ultimos 15 días
df_diferencia = filter_last_15_days(df_diferencia_except)
# Valida que el dataframe generado no este vacio
# hay_diferencias = df_diferencia.limit(1).count() > 0
hay_diferencias = True

# Si hay diferencias, se actualiza la tabla existente
if hay_diferencias:
    print("Se creó df_FCR_dif")

    # Cargar tabla Delta existente
    dt_origen = DeltaTable.forName(spark, delta_table_FCR)   

    # Upsert (merge) del DataFrame de diferencias
    dt_origen.alias("dest") \
        .merge(
            source=df_diferencia.alias("update"),
            condition="""
                dest.partner_coupon_row_id = update.partner_coupon_row_id AND 
                dest.partner_event_id = update.partner_event_id AND 
                dest.balance_type_id = update.balance_type_id AND 
                dest.operation_type_id = update.operation_type_id AND 
                dest.row_amount = update.row_amount
            """
        ) \
    ...
    print("Se actualizó exitosamente la base de Apuestas Deportivas")

else:
    print("La tabla se encuentra actualizada")


# COMMAND ----------

### Carga Completa Auxiliar de Tabla

# df_dataframe = df_AADD.withColumn("fecha_actualizacion", current_timestamp())
# print(delta_table_FCR)

# df_dataframe.write \
#         .mode("overwrite") \
#             .option("mergeSchema", "true") \
#                 .saveAsTable(delta_table_FCR)
...
if bl_Cargar_Diccionario_AADD:
    # Get DW2
  df_c2 = spark.read.parquet('/path/DW2/coupon')
  df_btn2 = spark.read.parquet('/path/DW2/balance_t_name')
  df_cosn2 = spark.read.parquet('/path/DW2/combination_status_name')
  df_otn2 = spark.read.parquet('/path/DW2/operation_t_name')

    # Get CORDOBA
  df_c3 = spark.read.parquet('/path/DW3/coupon')
  df_btn3 = spark.read.parquet('/path/DW3/balance_t_name')
  df_cosn3 = spark.read.parquet('/path/DW3/combination_status_name') 
  df_otn3 = spark.read.parquet('/path/DW3/operation_t_name')
...
# Path de Delta Table
#   Path PRD del Delta Table 
path_delta_table_Dic_prd = "path.productivo.diccionario"

#   Path DEV del Delta Table 
path_delta_table_Dic_dev = "path.desarrollo.diccionario"

#   Switch PROD/DEV
if PROD:
    delta_table_Dic = path_delta_table_Dic_prd
else:
    delta_table_Dic = path_delta_table_Dic_dev

if bl_Cargar_Diccionario_AADD:
    # Manejar el caso en que la tabla no exista
    try:
        # Intentar leer la tabla Delta
        df_origen_DicAADD = spark.read.table(delta_table_DicAADD)
        count = df_origen_DicAADD.select('diccionario_aadd').count()
        print(count)
...
if bl_Cargar_Diccionario_AADD:
    if not exist:
        ...
        df_Diccionario_AADD_Filtrado = df_Diccionario_AADD.exceptAll(df_origen_DicAADD)
        count = df_Diccionario_AADD_Filtrado.count()

        if count > 0:
            print(f'se actualizará la tabla {delta_table_DicAADD}')
            # Actualizar el DataFrame existente
            dt_origen_DicAADD = DeltaTable.forName(spark, delta_table_FCR)   
            # Upsert del DataFrame de Apuestas y Premios
            dt_origen_DicAADD.alias("dest")\
                .merge(source=df_Diccionario_AADD_Filtrado.alias("update"), condition="dest.partner_coupon_row_id = update.partner_coupon_row_id AND dest.partner_event_id = update.partner_event_id AND dest.balance_type_id = update.balance_type_id AND dest.operation_type_id = update.operation_type_id AND dest.row_amount = update.row_amount")\
                    .whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
        else:
            print(f'La tabla {delta_table_DicAADD} se encuentra actualizada')
